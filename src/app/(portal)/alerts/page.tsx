    ...
    <div className="space-y-8">
      <section>
        ...
        <p className="mt-2 text-slate-600">
          Alertas de segurança, disponibilidade e performance consolidados para{" "}
            ...
            <tbody className="divide-y divide-slate-100 bg-white">
              {alerts.map((alert) => (
                <tr key={`${alert.source}-${alert.title}-${alert.time}`}>
                  ...
                  <td className="px-4 py-3 text-slate-700">{alert.title}</td>
                  ...
                  <td className="px-4 py-3 text-slate-500">{alert.time}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-sm leading-6">
              Esta tela já está preparada visualmente, mas os alertas ainda são
              mockados. A próxima evolução será consumir dados normalizados de
              Wazuh e Zabbix.
            </p>
          </div>
        </div>
