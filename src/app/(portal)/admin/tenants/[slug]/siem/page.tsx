...
import { linkSiemAgentToAssetAction } from "@/app/actions/tenant-actions";
import { getTenantSiemAssetLinkSuggestions } from "@/lib/queries/admin-siem-asset-links";
...
export default async function TenantSiemPage({
  ...
  const linkData = await getTenantSiemAssetLinkSuggestions(slug);
    ...
    <div className="flex flex-col gap-4 border-b border-slate-100 p-6 lg:flex-row lg:items-center lg:justify-between">
      <div>
        <h2 className="text-lg font-bold text-slate-900">
          Vínculo com ativos do portal
        </h2>
        <p className="mt-1 text-sm text-slate-500">
          Sugestões de associação entre agentes SIEM e ativos cadastrados no
          inventário.
        </p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3 text-center text-xs">
        <div className="rounded-2xl bg-emerald-50 px-4 py-3">
          <div className="font-bold text-emerald-700">
            {linkData.summary.linked}
          </div>
          <div className="text-emerald-700">Vinculados</div>
        </div>

        <div className="rounded-2xl bg-blue-50 px-4 py-3">
          <div className="font-bold text-blue-700">
            {linkData.summary.suggested}
          </div>
          <div className="text-blue-700">Sugeridos</div>
        </div>

        <div className="rounded-2xl bg-slate-100 px-4 py-3">
          <div className="font-bold text-slate-700">
            {linkData.summary.unmatched}
          </div>
          <div className="text-slate-700">Sem match</div>
        </div>
      </div>
    </div>
    ...
    <div className="overflow-x-auto">
      <table className="w-full min-w-[1100px] text-left text-sm">
        <thead className="bg-slate-50 text-slate-500">
          <tr>
            <th className="px-4 py-3 font-semibold">Agente SIEM</th>
            <th className="px-4 py-3 font-semibold">IP</th>
            <th className="px-4 py-3 font-semibold">Status</th>
            <th className="px-4 py-3 font-semibold">Ativo vinculado</th>
        ...
        <tbody className="divide-y divide-slate-100 bg-white">
          {linkData.rows.map((row) => {
            const bindSuggestedAsset =
              row.suggestion && row.status === "SUGGESTED"
                ? linkSiemAgentToAssetAction.bind(null, {
                    tenantSlug: data.tenant.slug,
                    assetId: row.suggestion.asset.id,
                    wazuhAgentId: row.agent.wazuhAgentId,
                  })
                : null;
              ...
              <tr key={row.agent.id} className="align-top">
                ...
                <td className="px-4 py-4">
                  <span
                    className={[
                      ...
                      getStatusBadgeClass(row.agent.status),
                    ].join(" ")}
                  >
                    {getStatusLabel(row.agent.status)}
                  </span>
                </td>
                ...
                <td className="px-4 py-4">
                  {row.linkedAsset ? (
                    <div>
                      <div className="font-bold text-slate-900">
                        {row.linkedAsset.name}
                      </div>
                      <div className="mt-1 text-xs text-slate-500">
                        {row.linkedAsset.hostname ?? row.linkedAsset.ipAddress ?? "—"}
                      </div>
                    </div>
                  ) : (
                ...
                <td className="px-4 py-4">
                  {row.suggestion ? (
                    ...
                    <span className="text-emerald-700">Já vinculado</span>
                  ) : (
                    <span className="text-slate-400">Sem sugestão</span>
                  )}
                </td>
                ...
                <td className="px-4 py-4">
                  {bindSuggestedAsset ? (
                    <form action={bindSuggestedAsset}>
                      <button
                        type="submit"
